...
- **Protobuf contracts (`proto/`) are the source of transport shape** — avoid placing
  RPC-specific behavior directly in `api.py`.
- Config/settings canonical pattern: ADR-012.
- Codemod governance (ast-grep + make mod): ADR-014.

## Anti-Patterns / Gotchas

- Generated protobuf stubs are excluded from Ruff/type gates via per-file ignores —
  regenerate through the build, never hand-edit.

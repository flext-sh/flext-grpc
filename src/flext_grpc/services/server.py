...
class FlextGrpcServer(s):
    ...
    class GrpcServicer(flext_pb2_grpc.FlextGrpcServiceServicer):
        ...
        def Echo(
            ...
            response_model = m.Grpc.EchoResponse(
                message=request_model.message, server_id=self._server_id
            )
        ...
        def HealthCheck(
            ...
            response_result = FlextGrpcUtilities.Grpc.create_protobuf_message(
                ...
                {"status": response_model.status, "message": response_model.message},
                response=True,
            )
            return response_result.unwrap()
    ...
    class GrpcServerManager:
        ...
        def server_metrics(self, server: p.Grpc.Server) -> p.Result[p.Grpc.Payload]:
            """Get server metrics."""
        ...
        def start_server(self, server: p.Grpc.Server) -> p.Result[p.Grpc.Server]:
            ...
            if server_key in self._active_servers:
                result = r[p.Grpc.Server].fail(f"Server already running: {server_key}")
            else:
                try:
        ...
        def _start_new_server(
            self, server_key: str, server: p.Grpc.Server
            ...
            return self._activate_runtime_server(
                server_key, starting_server, grpc_server
            )

        def _create_bound_runtime_server(
            self, starting_server: p.Grpc.Server
        ...
        @staticmethod
        def _register_services(server_key: str, grpc_server: p.Grpc.GrpcServer) -> None:

...
class FlextGrpcServices:
    ...
    class Grpc:
        """Nested service implementations."""
        ...
        class MetricsCollector:
            ...
            def record_metric(self, key: str, value: t.ConfigValue) -> None:
                ...
                with self._lock:
                    normalized = FlextGrpcServices._MetricValueModel(value=value)
        ...
        class GrpcServerManager:
            ...
            def __init__(self) -> None:
                ...
                self._metrics = FlextGrpcServices.Grpc.MetricsCollector()
            ...
            def start_server(self, server: m.Grpc.Server) -> r[m.Grpc.Server]:
                ...
                try:
                    ...
                    for _service in starting_server.services:
                        real_servicer = FlextGrpcServices._create_real_servicer(
                            server_key
                        )
                        add_FlextGrpcServiceServicer_to_server(
        ...
        class GrpcClientManager:
            ...
            def __init__(self) -> None:
                ...
                self._connection_pool = FlextGrpcServices.Grpc.ConnectionPool(
                    max_size=20
                )
                self._metrics = FlextGrpcServices.Grpc.MetricsCollector()
        ...
        class GrpcStreamManager:
            ...
            def __init__(self) -> None:
                ...
                self._active_streams: MutableMapping[
                    str, FlextGrpcServices._StreamRuntimeState
                ] = {}
                self._metrics = FlextGrpcServices.Grpc.MetricsCollector()
            ...
            def create_stream(self, **kwargs: t.ConfigValue) -> r[m.Grpc.GrpcStream]:
                ...
                self._active_streams[stream_key] = (
                    FlextGrpcServices._StreamRuntimeState(
                        stream=stream,
                        created_at=time.time(),
                        buffer=FlextGrpcServices._new_stream_buffer(),
                    )
                )
            ...
            def send_data(
                ...
                try:
                    stream_state = FlextGrpcServices._StreamRuntimeState.model_validate(
                        stream_info
                    )
    ...
    def __init__(self) -> None:
        ...
        super().__init__()
        self._server_manager = FlextGrpcServices.Grpc.GrpcServerManager()
        self._client_manager = FlextGrpcServices.Grpc.GrpcClientManager()
        self._stream_manager = FlextGrpcServices.Grpc.GrpcStreamManager()
        self._metrics_collector = FlextGrpcServices.Grpc.MetricsCollector()
        self._resource_manager = FlextGrpcServices.Grpc.ConnectionPool(max_size=20)

...
class TestFlextGrpcTypes:
    ...
    def test_validate_target_edge_cases(self) -> None:
        ...
        # Test empty string
        assert not FlextGrpcTypes.GrpcValidation.validate_target("")

        # Test no colon
        assert not FlextGrpcTypes.GrpcValidation.validate_target("localhost")

        # Test empty host
        assert not FlextGrpcTypes.GrpcValidation.validate_target(":50051")

        # Test empty port
        assert not FlextGrpcTypes.GrpcValidation.validate_target("localhost:")

        # Test invalid host characters
        assert not FlextGrpcTypes.GrpcValidation.validate_target("invalid@host:50051")

        # Test port too low
        assert not FlextGrpcTypes.GrpcValidation.validate_target("localhost:0")

        # Test port too high
        assert not FlextGrpcTypes.GrpcValidation.validate_target("localhost:65536")

        # Test valid cases
        assert FlextGrpcTypes.GrpcValidation.validate_target("localhost:50051")
        assert FlextGrpcTypes.GrpcValidation.validate_target("127.0.0.1:8080")
        assert FlextGrpcTypes.GrpcValidation.validate_target("my-service.com:443")

    def test_parse_target_edge_cases(self) -> None:
        """Test edge cases for target parsing."""
        # Test various valid targets
        assert FlextGrpcTypes.GrpcValidation.parse_target("localhost:50051") == (
            "localhost",
            50051,
        )
        assert FlextGrpcTypes.GrpcValidation.parse_target("127.0.0.1:8080") == (
            "127.0.0.1",
            8080,
        )
        assert FlextGrpcTypes.GrpcValidation.parse_target("service.domain.com:443") == (
            "service.domain.com",
            443,
        )

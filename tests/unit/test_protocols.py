...
class Testp:
    ...
    def test_protocols_are_protocols(self) -> None:
        ...
        tm.that(is_protocol(p.Grpc.Server), eq=True)
        tm.that(is_protocol(p.Grpc.Client), eq=True)
        tm.that(is_protocol(p.Grpc.Streaming), eq=True)
        tm.that(is_protocol(p.Grpc.Service), eq=True)

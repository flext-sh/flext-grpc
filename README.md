...
FLEXT gRPC - High-Performance gRPC Services
...
2. Confirm parent MRO chain via `pyproject.toml` `dependencies` filtered by `flext-*` (excluding `flext-grpc` self).
...
- Public extensions exposed by this project: `FlextGrpcTypes`, `FlextGrpcValidationError`, `FlextGrpcModels`, `FlextGrpcProtocols`, `FlextGrpcError`, `FlextGrpcConnectionPool` (+13 more).
